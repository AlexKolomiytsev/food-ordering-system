        ...
        messages.forEach(paymentResponseAvroModel -> {
            try {
                if (PaymentStatus.COMPLETED == paymentResponseAvroModel.getPaymentStatus()) {
                    log.info("Processing successful payment response for order id: {}", paymentResponseAvroModel.getOrderId());
                ...
                log.error("Caught optimistic locking exception in PaymentResponseKafkaListener for order id: {}", paymentResponseAvroModel.getOrderId(), e);
            } catch (OrderNotFoundException e) {
                // NO-OP for OrderNotFoundException.
                log.error("Order not found for order id: {}", paymentResponseAvroModel.getOrderId(), e);
            }
        });
