...
public class OrderApprovalEventKafkaPublisher implements RestaurantApprovalRequestMessagePublisher {
    ...
    public void publish(OrderApprovalOutboxMessage orderApprovalOutboxMessage, BiConsumer<OrderApprovalOutboxMessage, OutboxStatus> outboxCallback) {
        ...
        log.info("Received OrderApprovalOutboxMessage for order id: {} and sagaId: {}", orderApprovalEventPayload.getOrderId(), sagaId);
        ...
        try {
            ...
            log.info("OrderApprovalEventPayload sent to Kafka for order id: {} and sagaId: {}", orderApprovalEventPayload.getOrderId(), sagaId);
        } catch (Exception e) {
            log.error("Error while sending OrderApprovalEventPayload to Kafka for order id: {} and sagaId: {} and error: {}", orderApprovalEventPayload.getOrderId(), sagaId, e.getMessage());
        }
    }
}
